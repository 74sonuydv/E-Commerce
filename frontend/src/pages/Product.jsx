...
const Product = () => {
  ...
  return productData ? (
    ...
    <div className='border-t pt-10 transition-opacity ease-in duration-500 backdrop-opacity-100'>
          ...
          <div className='flex flex-col gap-4 my-8'>
            ...
            <div className='flex gap-2'>
              {productData.sizes.map((item, index) => (
                <button onClick={() => setSize(item)} key={index} className={`border py-2 px-4 bg-gray-100 cursor-pointer ${item === size ? 'border-orange-400' : ''}`}>{item}</button>
              ))}
            </div>
          </div>
